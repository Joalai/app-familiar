...
const SF_SECTION_META={
 ...
 inbox:{title:'Bandeja',icon:'📥',view:'inbox'},shop:{title:'Compras',icon:'🛒',view:'shop'},packing:{title:'Maleta',icon:'🧳',view:'packing'},health:{title:'Salud',icon:'🩺',view:'health'},documents:{title:'Documentación',icon:'🪪',view:'documents'},
...
function counts(){
 ...
 return {
  ...
  inbox:(D?.inbox||[]).filter(x=>x.status==='pending').length,
...
function openQuickAdd(){
 const choices=[['inbox','📥','Bandeja familiar','Guardar ahora y clasificar después'],['shop','🛒','Compra','Añadir a la lista'],['voiceShop','🎙️','Compra por voz','Dictar productos'],['plans','🎂','Plan / cumpleaños','Añadir acontecimiento'],['health','🩺','Cita de salud','Añadir revisión o cita'],['routines','🔁','Rutina','Añadir actividad'],['packing','🧳','Maleta','Añadir objeto o viaje'],['races','🏃','Carrera','Ver objetivos y opciones']];
...
function searchRecords(q){
 ...
 (D?.inbox||[]).filter(x=>x.status==='pending').forEach(x=>push('📥',x.item_text,`${x.source||'app'}${x.created_by?' · '+x.created_by:''}`,'inbox',()=>openSection('inbox'),x.shared_url||''));
...
function attentionItems(){
 ...
 const inboxN=(D?.inbox||[]).filter(x=>x.status==='pending').length;if(inboxN)out.push({kind:'warn',title:'📥 Bandeja familiar',sub:`${inboxN} elemento${inboxN===1?'':'s'} por clasificar`,view:'inbox'});
